...
try:
    while True:
        ...
        client_socket.bind(("127.0.0.1", 38000))
        client_socket.listen(50)
        conn, addr = client_socket.accept()
        print("Connection established by Node 4")
        ...
        # Access and insert the first block in the database "user_information" and table node4
        ...
        if len(rows) == 0:
            ...
            try:
                with mysql_conn.cursor() as cursor:
                    sql = "INSERT INTO `node4` (`blockname`, `blockdata`) VALUES (%s, %s)"
            ...
            try:
                with mysql_conn.cursor() as cursor:
                    sql = "select blockname from `node4` "
                    cursor.execute(sql)
    ...
    print("In Node 4: ", ce.__traceback__)
