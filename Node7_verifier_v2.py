...
try:
    while True:
        ...
        client_socket.bind(("127.0.0.1", 41000))
        client_socket.listen(50)
        conn, addr = client_socket.accept()
        print("Connection established by Node 7")
        ...
        # Access and insert the first block in the database "user_information" and table node1
        ...
        if len(rows) == 0:
            ...
            try:
                with mysql_conn.cursor() as cursor:
                    sql = "INSERT INTO `node7` (`blockname`, `blockdata`) VALUES (%s, %s)"
            ...
            try:
                with mysql_conn.cursor() as cursor:
                    sql = "select blockname from `node7` "
                    cursor.execute(sql)
    ...
    print("In Node 7: ", ce.__traceback__)
