...
try:
    while True:
        ...
        print("Connection established by Node 6")
        ...
        if len(rows) == 0:
            ...
            try:
                with mysql_conn.cursor() as cursor:
                    sql = "INSERT INTO `node6` (`blockname`, `blockdata`) VALUES (%s, %s)"
            ...
            try:
                with mysql_conn.cursor() as cursor:
                    sql = "select blockname from `node6` "
                    cursor.execute(sql)
    ...
    print("In Node 6: ", ce.__traceback__)
